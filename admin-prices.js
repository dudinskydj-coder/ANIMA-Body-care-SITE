(function(){
  ...
  function renderStatus(){
    ...
    pills.push(pill("Локальный preview", state.localSnapshot ? "активен" : "нет", state.localSnapshot ? "ok" : "warn"));
    ...
    ui.saveLocalButton.disabled = !state.catalogApi || state.saving;
    ui.clearLocalButton.disabled = !state.localSnapshot || state.loading || state.saving;
    ...
    } else if(state.localSnapshot && state.localSnapshot.updatedAt){
      ui.saveMeta.textContent = `Локальный предпросмотр сохранён в этом браузере: ${new Date(state.localSnapshot.updatedAt).toLocaleString("ru-RU")}. Он виден только на этом устройстве и не затрагивает рабочий сайт.`;
  ...
  async function extractCatalogApi(){
    let response;
    try{
      response = await fetch("./Laser 4.html", { cache:"no-store" });
    } catch(_error){
      if(window.AnimaPricingRemote.isLocalEnvironment() && window.location.protocol === "file:"){
        throw new Error("Браузер не даёт читать Laser 4.html напрямую из file://. Для локальной проверки запусти папку через localhost.");
      }
      throw _error;
    }
    if(!response.ok){
  ...
  async function handleSaveLocal(){
    ...
    const saved = window.AnimaPricingRemote.writeLocalPricingSnapshot(snapshot);
    if(!saved){
      setMessage("Не удалось сохранить локальный предпросмотр в браузере.", "err");
      return;
    }
    state.localSnapshot = snapshot;
    state.dirty = false;
    setMessage("Локальный предпросмотр сохранён. Теперь открой Laser 4.html в этом же браузере и увидишь новые цены.", "ok");
    renderStatus();
  }

  async function handleClearLocal(){
    const cleared = window.AnimaPricingRemote.clearLocalPricingSnapshot();
    if(!cleared){
      setMessage("Не удалось очистить локальный предпросмотр.", "err");
      return;
    }
    state.localSnapshot = null;
    setMessage("Локальный предпросмотр очищен. Возвращаю базовый каталог.", "ok");
    await loadCatalog();
  }
  ...
  async function loadCatalog(){
    ...
    try{
      ...
      const localSnapshot = window.AnimaPricingRemote.readLocalPricingSnapshot();
      if(localSnapshot){
        window.AnimaPricingRemote.applySnapshotToPlanner(
          localSnapshot,
          api.plannerZonesByMode,
      ...
      if(localSnapshot){
        setMessage("Каталог загружен с локальным предпросмотром. Эти цены видны только в твоём браузере.", "ok");
  ...
  function bindEvents(){
    ...
    ui.saveLocalButton.addEventListener("click", handleSaveLocal);
    ui.clearLocalButton.addEventListener("click", handleClearLocal);
