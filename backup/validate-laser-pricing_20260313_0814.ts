...
const api = new Function(relevantSource)() as {
  ...
  plannerSummarySnapshot: (mode: string, selectedSet: Set<string>) => {
    ...
    items:Array<{ id:string; type:string; price:number; originalPrice:number; zoneIds:string[] }>;
  };
  plannerOrderItems: (mode: string, selectedSet: Set<string>) => Array<{ id:string; type:string; price:number; originalPrice:number; zoneIds:string[] }>;
};
...
function validateSelection(mode: string, selectedZoneIds: string[]) {
  ...
  for(const item of orderItems){
    ...
    for(const zoneId of item.zoneIds || []){
  ...
  if(mode === "frauen"){
    const ids = new Set(orderItems.map((item) => item.id));
    if(selectedSet.has("upperarms") && selectedSet.has("underarms") && selectedSet.has("hands")){
      if(!ids.has("smart_arms_complete_with_hands")){
    ...
    if(selectedSet.has("obersch") && selectedSet.has("untersch") && selectedSet.has("feet")){
      if(!ids.has("smart_legs_complete_with_feet")){
    ...
    if(promoZones.every((zoneId) => selectedSet.has(zoneId)) && selectedSet.size === promoZones.length){
      if(!ids.has("promo_intim_axilla_legs_bundle")){
