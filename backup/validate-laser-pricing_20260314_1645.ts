...
function collectLeafZoneCoverage(nodes: OrderNode[], coverage = new Map<string, number>()) {
  for(const node of nodes){
    ...
    if(node.type === "single" && node.id){
      coverage.set(node.id, (coverage.get(node.id) || 0) + 1);
      continue;
    }
...
function validateSelection(mode: string, selectedZoneIds: string[]) {
  ...
  if(mode === "frauen"){
    const exactArmsHandsSelection = selectedSet.size === 3
      && selectedSet.has("upperarms")
      && selectedSet.has("underarms")
      && selectedSet.has("hands");
    const exactLegsFeetSelection = selectedSet.size === 3
      && selectedSet.has("obersch")
      && selectedSet.has("untersch")
      && selectedSet.has("feet");
    if(exactArmsHandsSelection){
  ...
  if(mode === "herren"){
    const launchZones = ["axilla", "chest", "intim"];
    if(launchZones.every((zoneId) => selectedSet.has(zoneId)) && selectedSet.size === launchZones.length){
      if(!topLevelIds.has("axilla_chest_intim_bundle")){
        issues.push({ mode, selected:selectedKey, problem:"missing axilla_chest_intim_bundle on exact launch selection" });
      }
    }
    const backCompleteZones = ["lowerback", "upperback"];
    if(backCompleteZones.every((zoneId) => selectedSet.has(zoneId)) && selectedSet.size === backCompleteZones.length){
      if(!topLevelIds.has("back_complete_bundle")){
    ...
    if(chestBellyZones.every((zoneId) => selectedSet.has(zoneId)) && selectedSet.size === chestBellyZones.length){
      if(!topLevelIds.has("chest_belly_bundle")){
        issues.push({ mode, selected:selectedKey, problem:"missing chest_belly_bundle on exact chest+belly selection" });
      }
    }
    const intimAxillaZones = ["intim", "axilla"];
    if(intimAxillaZones.every((zoneId) => selectedSet.has(zoneId)) && selectedSet.size === intimAxillaZones.length){
      if(!topLevelIds.has("intim_axilla_bundle")){
        issues.push({ mode, selected:selectedKey, problem:"missing intim_axilla_bundle on exact intimate+underarms selection" });
      }
    }
    const bartkonturHalsZones = ["wangen", "chin"];
