(function(){
  ...
  const LOCAL_PRICING_SNAPSHOT_KEY = "anima_local_pricing_snapshot_v1";
  ...
  function isLocalEnvironment(){
    const protocol = normalizeText(window.location && window.location.protocol);
  ...
  function readLocalPricingSnapshot(){
    if(!hasLocalStorage()) return null;
    try{
      const raw = window.localStorage.getItem(LOCAL_PRICING_SNAPSHOT_KEY);
      if(!raw) return null;
  ...
  function writeLocalPricingSnapshot(snapshot){
    if(!hasLocalStorage()) return false;
    const payload = snapshot && typeof snapshot === "object" && snapshot.data
      ? snapshot
      : {
        key:"local_preview",
    ...
    try{
      window.localStorage.setItem(LOCAL_PRICING_SNAPSHOT_KEY, JSON.stringify(payload));
      return true;
    } catch(_error){
      return false;
    }
  ...
  function clearLocalPricingSnapshot(){
    if(!hasLocalStorage()) return false;
    try{
      window.localStorage.removeItem(LOCAL_PRICING_SNAPSHOT_KEY);
      return true;
    } catch(_error){
      return false;
    }
  ...
  window.AnimaPricingRemote = {
    ...
    readLocalPricingSnapshot,
    writeLocalPricingSnapshot,
    clearLocalPricingSnapshot,
